...
def save_final_png(image_bytes: bytes, output_path: Path, make_1080: bool) -> None:
    """Save generated bytes as PNG, optionally normalizing to 1080x1080."""
    if not make_1080:
...
def preview_output_path(output_folder: Path, source_path: Path) -> Path:
    return output_folder / f"{source_path.stem}_preview.png"
...
def process_catalog_jobs(
    ...
    with tempfile.TemporaryDirectory() as temp_name:
        ...
        for index, job in enumerate(jobs, start=1):
            ...
            existing_path = choose_output_path(output_folder, job.key, False)
            ...
            output_path = choose_output_path(output_folder, job.key, version_existing)
            ...
            for attempt in range(retry_count + 1):
                try:
                    ...
                    if output_style == "Infografia con texto exacto":
                        ...
                        save_final_png(image_bytes, base_path, False)
                        compose_infographic(base_path, output_path, job)
                    else:
                        save_final_png(image_bytes, output_path, make_1080)

                    expected_size = (1080, 1080) if make_1080 or output_style == "Infografia con texto exacto" else None
...
def generate_preview(
    ...
    preview_path = preview_output_path(output_folder, source_path)
    client = None if demo_mode else OpenAI(api_key=api_key)
    if demo_mode:
        model = "MODO DEMOSTRACION (sin IA)"
    ...
    with tempfile.TemporaryDirectory() as temp_name:
        ...
        if output_style == "Infografia con texto exacto" and job:
            ...
            save_final_png(image_bytes, preview_path, make_1080)
...
class BatchImageGeneratorApp:
    ...
    def _start_batch(self, skip_first_image: Path | None = None) -> None:
        ...
        if self.show_viewer.get():
            ...
            if skip_first_image:
                ...
                final_path = choose_output_path(output_folder, output_key, False)
    ...
    def _accept_preview_and_continue(self, window: Toplevel) -> None:
        ...
        final_path = choose_output_path(Path(self.output_folder.get().strip()), output_key, False)
